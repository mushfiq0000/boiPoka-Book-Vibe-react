...
const Bookdetails = () => {
  ...
  const { bookName, image, tags, review, totalPages, rating, category, publisher, yearOfPublishing} = singleBook;

  return (
    <div className="lg:flex flex-col md:flex-row  h-full
     p-6 gap-7">
      ...
      <div className="lg:w-1/2">
        ...
        <div className="divider"></div>
        <div>
          <p className="pb-5"><span className=" font-semibold">Review :</span> {review}</p>

          <div> <span className="font-semibold mr-3">Tag</span>
          {
            tags.map((tag) => (
              <button className="badge bg-gray-100 p-3 rounded-3xl border-0 text-green-500 mr-2">#{tag}</button>
            ))
          }
          <div className="divider"></div>
          <div className="mb-5">
